...
const SectionPrices: FC<SectionPriceProps> = ({ title, description }) => {
  return (
    ...
    >
      ...
      <div
        data-aos="fade-up"
        className="grid grid-cols-3 place-items-center w-8/12 gap-2 my-8"
      >
        {contentData.prices.map((price, index) => (
          <div
            key={index}
            className="w-full group overflow-hidden relative rounded-sm py-3 px-1 h-fit flex flex-col items-center hover:shadow-md justify-start bg-[#f3f4f6]  transition-all duration-300  hover:transition-all hoveer:duration-300  hover:transform hover:scale-105"
          >
            ...
            <div className="w-full h-fit py-1 flex flex-col items-center justify-start gap-x-2">
              {price.benefits.map((benefit, index) => (
                <p className="text-center" key={index}>{benefit.benefit}</p>
              ))}
            </div>
            <a href="" className="bg-[#8758ff] text-white px-5 font-semibold rounded-full mt-3">Lihat</a>
          </div>
        ))}
      </div>
    </section>
