const contentData: contentData = {
  features: [
    {
      logo: "/images/icons/money.svg",
      label: "Harga Kompetitif",
      description:
        "Buat undangan pernikahan tidak pernah semudah dan semurah ini",
    },
    {
      logo: "/images/icons/headset.svg",
      label: "Layanan Terbaik",
      description:
        "Perlu bantuan ? tenang tim support Kami selalu bisa diandalkan",
    },
    {
      logo: "/images/icons/thumbs.svg",
      label: "Fitur Lengkap",
      description:
        "Fitur yang dapat diandalkan mulai dari proses pembuatan undangan hingga proses membagikan undangan",
    },
    {
      logo: "/images/icons/lightning.svg",
      label: "Proses Cepat",
      description:
        "Proses pembuatan undangan hanya memerlukan waktu 5 menit serta dapat diubah kapanpun",
    },
  ],
  featureItems: [
    {
      logo: "/images/iconsitems/book.svg",
      label: "Buku Tamu",
      description:
        "Dapat menerima ucapan dan doa serta status kehadiran dari tamu undangan",
    },
    {
      logo: "/images/iconsitems/mail.svg",
      label: "Amplop Digital",
      description: "Tamu dapat memberikan amplop langsung secara digital",
    },
    {
      logo: "/images/iconsitems/map.jpg",
      label: "Petunjuk Lokasi",
      description: "Dapat menunjukkan dan mengarahkan tamu ke lokasi acara",
    },
    {
      logo: "/images/iconsitems/gallery.svg",
      label: "Gallery Foto",
      description: "Bagikan momen bahagia Kamu kepada tamu undangan",
    },
    {
      logo: "/images/iconsitems/music.svg",
      label: "Background Music",
      description: "Hiasi undangan pernikahan online dengan musik kesukaanmu",
    },
    {
      logo: "/images/iconsitems/history.svg",
      label: "Hitung Mundur",
      description: "Atur sesi kedatangan untuk setiap tamu yang diundang",
    },
  ],
  stepCreateInvitations: [
    {
      label: "Registrasi",
      description:
        "Buat akun baru dengan cara mengisikan email dan password atau daftar menggunakan akun Google.",
    },
    {
      label: "Isi Informasi",
      description:
        "Isi informasi mengenai mempelai, lokasi dan waktu acara, pilih desain undangan dan upload foto ke galeri.",
    },
    {
      label: "Bagikan",
      description:
        "Sebarkan undanganmu kepada kerabat atau teman yang ingin di undang dan selamat berbahagia.",
    },
  ],
  designs: [
    {
      image: "/images/coverinvitations/",
      name: "Black Javanese",
      tier: "Free",
    },
    {
      image: "/images/coverinvitations/",
      name: "Black Javanese",
      tier: "Free",
    },
    {
      image: "/images/coverinvitations/",
      name: "Black Javanese",
      tier: "Free",
    },
    {
      image: "/images/coverinvitations/",
      name: "Black Javanese",
      tier: "Free",
    },
  ],
  prices: [
    {
      tier: "Gratis",
      price: 0,
      benefits: [
        {
          benefit: "tulBetul",
        },
        {
          benefit: "Edit tanpa batas",
        },
        {
          benefit: "Quotes",
        },
        {
          benefit: "Amplop Digital",
        },
        {
          benefit: "Hitung Mundur",
        },
        {
          benefit: "Background Musik",
        },
        {
          benefit: "Masa Aktif 2 Hari",
        },
      ],
    },
    {
      tier: "Diamond",
      price: 125000,
      benefits: [
        {
          benefit: "Edit tanpa batas",
        },
        {
          benefit: "Apaya",
        },
        {
          benefit: "Quotes",
        },
        {
          benefit: "Amplop Digital",
        },
        {
          benefit: "Hitung Mundur",
        },
        {
          benefit: "Background Musik",
        },
        {
          benefit: "Masa Aktif 2 Hari",
        },
        {
          benefit: "Masa Aktif 2 Hari",
        },
        {
          benefit: "Masa Aktif 2 Hari",
        },
      ],
    },
    {
      tier: "Silver",
      price: 75000,
      benefits: [
        {
          benefit: "Gmaps",
        },
        {
          benefit: "Edit tanpa batas",
        },
        {
          benefit: "Quotes",
        },
        {
          benefit: "Amplop Digital",
        },
        {
          benefit: "Hitung Mundur",
        },
        {
          benefit: "Background Musik",
        },
        {
          benefit: "Masa Aktif 2 Hari",
        },
      ],
    },
  ],
  testimonials: [
    {
      username: "UcupzGg123",
      coverinvitation: "bg-green-300",
      profileuser: "/asdjkj",
      commentuser:
        "Ga nyangka buat undangan pernikahan keren gak pake biaya gedecuma sekali bayar bisa dibagiin ke banyak tamu sekaligus makasih weddingnyong",
      invitationname: "BlackJavanese",
    },
    {
      username: "Soeyono",
      coverinvitation: "bg-green-300",
      profileuser: "/asdjkj",
      commentuser:
        "Makasih weddingnyong aku jadi ngirit budget pernikahan cuma beli sekali bisa dishare kesemuanya",
      invitationname: "HappyPeach",
    },
    {
      username: "Asep",
      coverinvitation: "bg-green-300",
      profileuser: "/asdjkj",
      commentuser: "Undangan masa depan!!!",
      invitationname: "Pasteliens",
    },
    {
      username: "Adrean",
      coverinvitation: "bg-green-300",
      profileuser: "/asdjkj",
      commentuser: "ASDFGHJKL MAKASIH WEDDING NYONG!!",
      invitationname: "PinkyWingky",
    },
  ],
  invitations: [
    {
      slug: "weddingnyong.com/invitation/ucuplovesurti",
      guest: [
        {
          guestname: "Adrian",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Asep",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Aceng",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
      ],
    },
    {
      slug: "weddingnyong.com/invitation/adrianlovevinka",
      guest: [
        {
          guestname: "Adrian",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Asep",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Aceng",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Adrian",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Asep",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
        {
          guestname: "Aceng",
          whatsapp: "+62882687287",
          attedance: false,
          qr: "sjkdkajsd",
        },
      ],
    },
  ],
};

export default contentData;
